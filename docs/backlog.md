...
> Última atualização: 2026-09-09 (Rev.156: GifPacket bridge fidelity + p2 family discriminator closed)
...
- **Item 2b**: ~~atmosfera/texturas/spawn costa~~ **DONE (Rev.169, 2026-09-10)** — `ClipBridge::bestFloorPoint()` (maior piso andável com vizinho 4-neighbour; exclui topo de parede 1 célula) + fallback de spawn em `main.cpp` (st02a `[highest walkable floor] → ok`, era viewer-only); `texForName` com fallback pro bundle compartilhado `assets/scene/texture/` (dedupe log "room dir + shared"); céu placeholder de luz do dia quando a sala não tem `sky.tm2` (host presentation, reversível), clear = cor do horizonte, gradient sempre desenhado (st17a mantém sky real amostrado); **as 5 texturas ausentes do st02a foram reextraídas de `DFDATAS/DATA.DF` do PAL ISO** (LBA 19771, registro `STGST02A.DF` off u32 em +0x20/size +0x24, payload raw-deflate, container 518 entradas; mapeamento `block1←06a_block1`, `abe2/3←2b_kabe2/3`, `a01_d/a02_d←renga01_d/02_d`) → **zero missing textures** no `--room st02a`. CTest 28/29 (baseline headless `opengl_backend`). Ver nota `research/native/rev169-atmosphere-texture-fallback-and-coastal-spawn.md`.
- **Item 2c**: ~~demo duas salas com troca REAL de resident set~~ **DONE (Rev.170, 2026-09-10)** — `--room <primary> --pair <companion>` (st00a como principal, st02a `parseRoom` na mesma `SceneAssetStore`); atravessar `door.p2o` faz `requestScene(0x2B)`+`execute()` de verdade: `DOOR OPEN st00a -> st02a`, `transition execute ok (scene 0x2B, 23 host GObjs)`, render sustenta 68/29,462 tris → 54/34,195 tris, zona reversa reinstalada pós-swap (`door zone st02a → 0x0F`), shot `/tmp/rev170-swap3.ppm`. Zonas de porta encaixam no **piso standable de 5 cantos** (mesmo predicado do spawn; só `floorHeightAt` deixava o boy ~60u da porta). Fallback de colisão/fit/anchor p/ sala sem `_p1` passou a usar o **MAIOR mesh carregado** (antes: primeiro piece decorativo = grade 8×7 do flare; agora st02a → `st02a_p2.p2o` 73×72). **Aberto/limitação**: `st02a_p1.p2o` continua sem parse (variante p2-family; região de faces em 0x1150 não casa com `[N,ffff x7]` da Rev.151) → demo one-way. CTest 29/30. Ver nota `research/native/rev170-two-room-door-transition.md`.
- **Item 2d**: ~~matriz multi-room verifica generalização~~ **DONE (Rev.172, 2026-09-11)** — 12/12 transições direcionais headless entre st00a/st02a/st13b/st18a (`DOOR OPEN X -> Y` + `transition execute ok` em cada run); pipeline é data-driven desde Rev.170, nenhuma mudança de código. Snaps das 4 portas funcionais e catálogo das 10 salas com peça de porta (4 OK, 5 sem snap no raio 400: st03t/st04b/st06a/st13c/st22a, st05b sem testar). Ver nota `research/native/rev172-room-transition-multi-room-matrix.md`.
...
### [x] [SQUAD-TOOLING | rev.103b | 2026-08-25]
initSceneGObj decompilation + Capstone skipdata fix + toolchain inventory

- **initSceneGObj (0x1B76F8, 2088B)**: decompilada a 100% byte-exact via `asm_source_score.py`. 522 instruções, 6 `.word` directives para R5900-specific instructions (3x `div` 3-operand, 1x `mult ac3`, 2x `c.olt.s`). Ficheiro: `src/entity/asm/initSceneGObj.s` (576 linhas).
- **Capstone skipdata fix**: `asm_source_score.py` — adicionado `md.skipdata = True` ao `disassemble_mips64()`. Quando Capstone encontra instruções R5900 não reconhecidas, emite `.byte` → `insn_to_asm()` converte para `.word`. Sem este fix, o pipeline parava em 22/522 instruções.
- **Toolchain inventory no AGENTS.md**: secção "Required toolchain (pre-flight check)" com tabela de ferramentas críticas, pacotes Python, R5900 assembler limitations, e verification command. Previne que agents futuros percam tempo sem dependências.
- **elf_replacer.py paths corrigidos**: `/home/peter/` hardcoded → relativos ao projecto via `Path(__file__)`.
- **ee-gcc instalado**: `toolchain/ee-gcc2.9-991111-01/bin/ee-gcc` via decomp.me releases (2.1MB tar.xz).
...
| rev.103 | 2026-08-25 | SQUAD-RUNTIME | **Runtime session: boot→Yorda→bridge→first save.** 86,339 events captured. 14 world_state transitions (0x01→0x29→0x2a→0x2b→0x2d→0x28→0x03→0x05→0x04→0x05→0x04→0x06→0x07). 13 unique world states. 8 BSS dispatch slots mapped (0x00677DD8-0x0067E458), each dominant per room. a1=0x00000008 = system DL bootstrap (3.3%). isys_gobj_proc_add: 72% adds, 28% removes; t1=0x1800 universal (99.95%). init_scene_gobj: 5 system entries + room-specific entities (28-170 per room). ws=0x29 anomalous (41.7% mask=0). ELF extracted from BIN via existing elf_extractor.py tool. Nota: research/elf/ghidra-rev103-isysgobj-runtime-session-yorda-bridge-save.md. |
...
| rev.142 | 2026-09-06 | SQUAD-PORT | **PS2O (`.p2o`) geometry decode.** Vertex positions = 4 floats LE `(x,y,z,1.0)` from +0x20 (16 B/vertex, Y-up, floor at Y≈0); face data = 16 B records `[c,t,0,a,1,b,0,0]` in strips framed by `0xFFFF` at the `c` position. Full p2 scan into `/tmp/st00a_p2_faces.obj`. Nota: research/elf/rev142-p2o-vertex-layout-and-face-record-structure.md. |
| rev.143 | 2026-09-06 | SQUAD-PORT | **Texturized room render.** `ico_native` auto-loads `.p2o` mesh (`170_st00a_p1.p2o`: 13,877 verts, 15,161 tris, 3 submeshes, 13,070 UVs) + `.tm2` texture from `native/assets/`. Material↔TM2 by name (7 materials). `dfdatas_unpack.py` extracts `.p2o`/`.tm2` from PAL `.DF` containers. `run_native_demo.sh`. 21/22 CTest (opengl_backend_test segfaults headless). |
| rev.144 | 2026-09-07 | SQUAD-PORT | **PS2O TYPE 0x01 semantics resolved.** TYPE 0x01 decodes exactly like TYPE 0x00 (a-col spine); `b` (u16[5]) never generates triangles; boundary-edge ratio proves it: p1 M-A bnd_ratio 0.025 (closed mesh, exactly 15,161 tris) vs 0.375–0.714 for all `b`-consuming models; p2 0.126 vs 0.475+. No `Ps2oMesh` decoder change needed. Nota: research/elf/rev144-p2o-type01-resolved.md. |
| rev.145 | 2026-09-07 | SQUAD-ARCH | **GifPacket.o reconciliation false positives.** Of 38 `GifPacket.o` symbols in `pal_usa_symbol_map.csv`, only `gif_Sprite` (0x00110948) is byte-verified same-VA on USA; 36 fuzzy `candidate` rows pointed to wrong USA functions (`gif_EndPacket`→0x13F9D0=`_iosOmMain`, etc.) → flipped to `rejected`; `gif_Point` stays unmatched. The module IS present in USA at ~0x10FD40-0x112A30 (GIF buffer 0x4C7710), so the PAL VAs are near-correct but per-function boundaries beyond `gif_Sprite` are unverified. Native `GifPacketBridge` is a semantic name-based bridge (9 tests), not byte-exact. Treat any `candidate` with `sz_diff>0` or conflicting verified targets as suspicious. Nota: research/elf/rev145-gifpacket-reconciliation-false-positives.md. |

| rev.146 | 2026-09-07 | SQUAD-ARCH | **GifPacket.o USA boundaries closed byte-exact (18 .s) + PAL-vs-USA tail divergence.** Discovery: PAL `symbol_addrs` VAs are **internal labels inside real PAL functions, not starts** (e.g. `gif_EndPacket` PAL 0x10FD98 = +0x60 inside PAL fn 0x10FD38; `gif_Sprite` PAL 0x110948 = +0x140 inside PAL fn 0x110808). Content-identical PAL→USA at delta **+0x78** for the whole GIF body (only buffer-base `lui 0x4f→0x4c` and one `jal` differ; verified 100/103 words on 412 B sample). 17 body fns + `gif_test` = 18 byte-exact `.s` at `src/core/asm/gif_*.s`, all `assemble_and_verify` 100%. **Correção Rev.145:** USA `gif_Sprite` = 0x110790 (428B), NÃO 0x110948 (which is `gif_SpriteSensitive`, 412B). Multiple PAL labels share one USA fn (`EndPacket`+`EndPacketPath1`→0x10FCC0; `Init`+`StartPacketPri`+`StartPacketPriPath1`+`SetGsReg`→0x111CF0; etc). **GIF tail diverges:** PAL-only `gif_SetDrawEnviroment`/`gif_DrawPolyF4`/`gif_LineOffset`/`gif_SetAlpha`/`gif_MoveImage`/`gif_SetZTest`/`gif_SetZWrite`/`gif_SetHalfOffset` have NO op_seq or normalized-byte match in USA `.text` (USA uses own 0x1123C0/0x11260C GS-reg code); re-aligns at 0x112CEC(PAL)=0x1128FC(USA). Tool: `asm_source_score.py` `bbit` emit branch now covers `bbit1` (bbit132, needed by `gif_test`). CSV 25 gif rows → `verified`. Nota: research/elf/rev146-gifpacket-usa-boundaries-byte-exact.md. |
| rev.148 | 2026-09-07 | SQUAD-PORT | **Native gap analysis (playable milestones).** Validation of `instrucoes-agente-input-colisao-boy.md` plan against the tree; 2 factual corrections recorded. Analysis-only revision. Nota: research/native/rev148-playable-gap-analysis.md. |
| rev.149 | 2026-09-07 | SQUAD-PORT | **Fase A+B: walkable character on real mesh.** ClipBridge collision (4-sided clip per room) + `PlayerController`; player spawns on actual p1 floor, WASD walk with delta-time integration, hit mark. 22/23 CTest. Nota: research/native/rev149-walkable-character-clipbridge-real-mesh.md. |
| rev.150 | 2026-09-08 | SQUAD-PORT | **BoyController semantic port.** Replaces placeholder with semantic port of verified PS2 BOY state machine (boy_hA/hB/hC, 0x1C1A98/0x1C1DD8/0x1C1F58); 25/25 CTest. Nota: research/native/rev150-boy-controller-semantic-port.md. |
| rev.151 | 2026-09-08 | SQUAD-PORT | **p1 face/UV decode correction (byte-validated).** Record is **not** `[c,t,0,a,1,b,0,0]`; strip = 16 B header `[N, 0xFFFF x7]` (N = literal record count; count×N reproduces record total exactly) + records `[1,0,a,s,m,b,u,f]`: pos=`u16[2]` (max 7792), **UV index=`u16[4]`** (max 13069 = NUV-1; EXACT full coverage 13070/13070), **material=`u16[7]`** (exactly 7 values 0-6 == the 7 embedded names), stream=`u16[3]` (const per strip, NOT material), `u16[6]` (10 distinct values, NOT material). Tris = cascade (N-2 per strip). p1: 7,877 strips / 15,007 tris / UV edge spread 0.1645, renders 15019 tris with brick texture. Old "UV = k + offset[f]" heuristic WRONG → replaced by direct `u16[4]` in `Ps2oMesh.cpp`. p2 wall family still needs its own discriminator. Nota: research/native/rev151-p1-face-uv-decode-and-texturized-castle.md. |
| rev.153 | 2026-09-09 | SQUAD-PORT | **Unified GIF command/packet, kanban loader seam, strip synthesis.** Sky + strips ride one semantic GIF packet per frame; `KanbanSceneLoader` (kanban.c) wired for scene composition + runtime load; strip topology synthesized for strip-less pieces. Nota: research/native/rev153-unified-gif-command-packet-kanban-loader-seam-strip-synthesis.md. |
...
| rev.170 | 2026-09-10 | SQUAD-PORT | **Two-room REAL door swap (item 2b) — milestone one-way.** `--room <primary> --pair <companion>`; `requestScene(0x2B)` swap real: 68/29.462 tris (st00a) → 54/34.195 (st02a); `SceneAssetStore` compartilhado via `parseRoom`; ClipBridge/fit/sky/door por sala; zonas de porta encaixadas no **piso standable5** (mesmo predicado do spawn); fallback de colisão = **MAIOR malha carregada** (st02a_p1 falhava → p2 21.586 tris). Swap verificado `DOOR OPEN st00a -> st02a`. One-way (porta st02a nunca encaixava). CTest 29/30. Nota: research/native/rev170-two-room-door-transition.md. |
| rev.171 | 2026-09-11 | SQUAD-PORT | **Room-family variant decode + TWO-WAY door (item 2c).** `loadCharacterMesh` pula regiões OBJH `nv==0` e regiões sem header de strip em vez de abortar o arquivo → `st02a_p1.p2o` (15.552 verts / 19.652 tris / 67 submeshes) e `st02a_p3.p2o` (2.488/2.851/16) carregam; ClipBridge do st02a agora vem do interior REAL (p1): _room st02a clipBridge ok grid=54x65 blocked=2033_, render 57 calls / 56.698 tris (vs 54/34.195 fallback-p2). Raio de snap da porta 150→400 (heuristica host; centro AABB da porta fica ~300 u do piso standable5; dois terraços de penhasco). **Transição verificada NOS DOIS SENTIDOS** headless: `DOOR OPEN st00a -> st02a` e `DOOR OPEN st02a -> st00a` + `transition execute ok (scene 0x2B, 23 host GObjs)` + reinstalação da zona reversa. CTest 29/30 (só `opengl_backend` headless). Nota: research/native/rev171-st02a-p1-decode-and-two-way-door.md. |
| rev.172 | 2026-09-11 | SQUAD-PORT | **Multi-room transition matrix — pipeline generaliza (12/12).** Verificação headless `--room/--pair` de TODAS as 12 transições direcionais entre st00a/st02a/st13b/st18a: cada run loga `DOOR OPEN X -> Y` + `transition execute ok` (swap real `requestScene(0x2B)`, 23 host GObjs). Nenhuma mudança de código (já era data-driven); prova que o swap é por sceneId, não por pares aprendidos (st13b↔st18a funciona sem st00a). Snaps: st00a (16.67,15.29), st02a (-210,-265), st13b (1287.83,-958.462), st18a (~0,70.34). Catálogo das 10 salas com peça de porta: 4 funcionais + 5 sem snap no raio 400 (st03t/st04b/st06a/st13c/st22a — centro AABB longe do piso standable5 do interior) + st05b sem testar. CTest 29/30 (só `opengl_backend` headless). Nota: research/native/rev172-room-transition-multi-room-matrix.md. |
