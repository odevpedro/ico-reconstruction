...
52. **Native engine next:** implementar re-link GObj→mesh por sala no host usando os handlers verificados da Rev.158 (substituir `m_sceneGObjs[i % size]` de Rev.155 por um mapa `GObjHandle→handler`); repair `draw2DUVStripG` per-vertex UV carrier in `RenderCmd` (documented limitation).
